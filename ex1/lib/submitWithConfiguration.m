function submitWithConfiguration(conf)
  ...
  try
    response = submitParts(conf, email, token, p);
  catch
    e = lasterror();
    fprintf('\n!! Submission failed: %s\n', e.message);
    fprintf('\n\nFunction: %s\nFileName: %s\nLineNumber: %d\n', ...
...
function response = submitParts(conf, email, token, p)
  body = makePostBody(conf, email, token, p);
...
function body = makePostBody(conf, email, token, p)
  bodyStruct.assignmentSlug = conf.assignmentSlug;
  bodyStruct.submitterEmail = email;
  bodyStruct.secret = token;
  bodyStruct.p = makePartsStruct(conf, p);

  opt.Compact = 1;
  body = savejson('', bodyStruct, opt);
end

function partsStruct = makePartsStruct(conf, p)
  for part = p
    partId = part{:}.id;
...
function showFeedback(p, response)
  fprintf('== \n');
